...
import {setAsyncStorage} from '../Functions/AsyncStorageFunction';
...
export function* getCategoriesSaga(action: any): any {
  try {
    const {accessToken} = action;
    setNetworkActivityStatusBar(true);
    const response = yield call(userApi.getCategories, accessToken);
    setNetworkActivityStatusBar();
    if (response.ok && response.status === 200) {
